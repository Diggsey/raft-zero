...
use crate::commit_state::CommitStateExt;
...
use crate::storage::{LogRange, LogRangeOrSnapshot, Storage, StorageExt};
...
impl<A: Application> LogReplication<A> {
    ...
    pub(super) async fn fill_buffer(
        ...
        shared.commit_state.set_match_index(
            shared.node_id,
            self.prev_log_index(),
            self.prev_log_term,
        );
    ...
    pub(super) fn flush_buffer(&mut self, shared: &mut SharedState<A>) {
        ...
        shared
            .this
            .await_append_entries_response(count, res, shared.timer_token.inc(), deadline);
    }
